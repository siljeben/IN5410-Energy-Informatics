class Appliance():
    def __init__(self, name: str, shiftable: int, usage_h: int, daily_usage_kWh: float, alpha: int, beta: int) -> None:
        ...
        Args:
            ...
            shiftable (int): 0 - if it's non-shiftable, 1 if it's shiftable and 2 if it's an optional appliance (may be shiftable, but not required)
        ...
        if shiftable == 0 and beta - alpha > usage_h: 
        ...
        if shiftable == 1 and beta - alpha == usage_h: 
