...
from data_processing import get_sliding_window_input_output, get_sliding_window_input_test_data
...
X_train_svr, y_train_svr = get_sliding_window_input_output(power_train, window_size=window_size_svr)
...
X_train_lr, y_train_lr = get_sliding_window_input_output(power_train, window_size=window_size_lr)
...
# kNN model
window_size_knn=2
X_test_knn = get_sliding_window_input_test_data(
    y_sol, power_train, window_size=window_size_knn
)
X_train_knn, y_train_knn = get_sliding_window_input_output(power_train, window_size=window_size_knn)
...
print(f"Errors for the methods;\n"
      ...
      f"kNN: {knn_error}, window_size= {window_size_knn}")

#SVR: 0.1282092712257549, window_size= 83
#LR: 0.12223004615190537, window_size= 100
#kNN: 0.13011823376168988, window_size= 2

plot_timeseries(test_pred, [y_sol, svr_y_pred], ['True power output', 'Predicted power output'], 'SVR model', 'Power output')

plot_timeseries(test_pred, [y_sol, lr_y_pred], ['True power output', 'Predicted power output'], 'LR model', 'Power output')

plot_timeseries(test_pred, [y_sol, knn_y_pred], ['True power output', 'Predicted power output'], 'KNN model', 'Power output')
...
knn_result_df.to_csv('predictions/task3/ForecastTemplate3-kNN.csv', index=False)
"""